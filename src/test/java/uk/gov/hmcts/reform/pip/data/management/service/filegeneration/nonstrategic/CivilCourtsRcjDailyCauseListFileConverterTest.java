...
class CivilCourtsRcjDailyCauseListFileConverterTest {
    ...
    void setup() throws IOException {
        try (InputStream inputStream = getClass()
            .getResourceAsStream("/mocks/non-strategic/civilCourtsRcjDailyCauseList.json")) {
    ...
    @Test
    void testCivilCourtsRcjDailyCauseListFileConversionInEnglish() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/civilCourtsRcjDailyCauseList.json")) {
        ...
        Map<String, String> metadata = Map.of(CONTENT_DATE_METADATA, CONTENT_DATE,
                                              ...
                                              LIST_TYPE_METADATA, CIVIL_COURTS_RCJ_DAILY_CAUSE_LIST.name(),
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Civil Courts at the Royal Courts of Justice Daily Cause List");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Requests from the media and others, including legal bloggers, should be made to Civil Court "
                          + "at the Royal Courts of Justice at mediaenquiries.civilcourtsatthercj@justice.gov.uk.");
    ...
    @Test
    void testCivilCourtsRcjDailyCauseListFileConversionInWelsh() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/cy/non-strategic/civilCourtsRcjDailyCauseList.json")) {
            languageResource = new ObjectMapper().readValue(
                Objects.requireNonNull(languageFile).readAllBytes(), new TypeReference<>() {
                });
        }
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Rhestr Achosion Dyddiol Llys Sifil yn y Llysoedd Barn Brenhinol");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Dylid gwneud ceisiadau gan y cyfryngau ac eraill, gan gynnwys blogwyr cyfreithiol, i'r Llys "
                          + "Sifil yn y Llysoedd Barn Brenhinol yn mediaenquiries.civilcourtsatthercj@justice.gov.uk.");
    ...
    @Test
    void testCivilCourtsRcjDailyCauseListTableContents() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/civilCourtsRcjDailyCauseList.json")) {
    ...
    @Test
    void testCivilCourtsRcjDailyCauseListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/civilCourtsRcjDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, CIVIL_COURTS_RCJ_DAILY_CAUSE_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, CIVIL_COURTS_RCJ_DAILY_CAUSE_LIST, metadata,
                                                     excelFile);
    ...
    @Test
    void testCivilCourtsRcjDailyCauseListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/civilCourtsRcjDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, WELSH,
