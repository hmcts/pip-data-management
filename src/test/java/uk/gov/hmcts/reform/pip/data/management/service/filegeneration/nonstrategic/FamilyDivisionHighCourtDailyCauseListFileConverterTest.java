...
class FamilyDivisionHighCourtDailyCauseListFileConverterTest {
    ...
    void setup() throws IOException {
        try (InputStream inputStream = getClass()
            .getResourceAsStream("/mocks/non-strategic/familyDivisionHighCourtDailyCauseList.json")) {
    ...
    @Test
    void testFamilyDivisionHighCourtDailyCauseListFileConversionInEnglish() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/familyDivisionHighCourtDailyCauseList.json")) {
        ...
        Map<String, String> metadata = Map.of(CONTENT_DATE_METADATA, CONTENT_DATE,
                                              ...
                                              LIST_TYPE_METADATA, FAMILY_DIVISION_HIGH_COURT_DAILY_CAUSE_LIST.name(),
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Family Division of the High Court Daily Cause List");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Any application made after 1 March 2022 will be listed as an in person hearing unless an "
                          + "application for it to be heard remotely has been made prior to the hearing and granted "
                          + "by a judge.");
    ...
    @Test
    void testFamilyDivisionHighCourtDailyCauseListFileConversionInWelsh() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/cy/non-strategic/familyDivisionHighCourtDailyCauseList.json")) {
            languageResource = new ObjectMapper().readValue(
                Objects.requireNonNull(languageFile).readAllBytes(), new TypeReference<>() {
                });
        }
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Rhestr Achosion Dyddiol Adran Deulu yr Uchel Lys");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Bydd unrhyw gais a wneir ar ôl 1 Mawrth 2022 yn cael ei restru fel gwrandawiad wyneb yn wyneb "
                          + "oni bai bod cais i'w wrando o bell wedi'i wneud cyn y gwrandawiad a'i ganiatáu gan "
                          + "farnwr.");
    ...
    @Test
    void testFamilyDivisionHighCourtDailyCauseListTableContents() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/familyDivisionHighCourtDailyCauseList.json")) {
    ...
    @Test
    void testFamilyDivisionHighCourtDailyCauseListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/familyDivisionHighCourtDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, FAMILY_DIVISION_HIGH_COURT_DAILY_CAUSE_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, FAMILY_DIVISION_HIGH_COURT_DAILY_CAUSE_LIST, metadata,
                                                     excelFile);
    ...
    @Test
    void testFamilyDivisionHighCourtDailyCauseListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/familyDivisionHighCourtDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, WELSH,
