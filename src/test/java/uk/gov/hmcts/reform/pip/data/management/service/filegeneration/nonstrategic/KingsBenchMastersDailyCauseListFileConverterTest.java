...
class KingsBenchMastersDailyCauseListFileConverterTest {
    ...
    void setup() throws IOException {
        try (InputStream inputStream = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchMastersDailyCauseList.json")) {
    ...
    @Test
    void testKingsBenchMastersDailyCauseListFileConversionInEnglish() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/kingsBenchMastersDailyCauseList.json")) {
        ...
        Map<String, String> metadata = Map.of(CONTENT_DATE_METADATA, CONTENT_DATE,
                                              ...
                                              LIST_TYPE_METADATA, KINGS_BENCH_MASTERS_DAILY_CAUSE_LIST.name(),
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("King’s Bench Masters Daily Cause List");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Any media representative (or any other member of the public) wishing to attend a remote "
                          + "hearing should provide an email address and contact number to be sent an appropriate "
                          + "link for access.");

        softly.assertThat(document.getElementsByClass(LINK_CLASS))
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .hasSize(3);
        ...
        softly.assertThat(document.getElementsByClass(LINK_CLASS).get(2)
                              .getElementsByTag("a").get(0)
                              .attr(HREF))
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .isEqualTo("https://www.gov.uk/guidance/queens-bench-hearing-and-trial-dates");
    ...
    @Test
    void testKingsBenchMastersDailyCauseListFileConversionInWelsh() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/cy/non-strategic/kingsBenchMastersDailyCauseList.json")) {
            languageResource = new ObjectMapper().readValue(
                Objects.requireNonNull(languageFile).readAllBytes(), new TypeReference<>() {
                });
        }
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Rhestr Achosion Dyddiol Meistri Mainc y Brenin");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Dylai unrhyw gynrychiolydd y cyfryngau (neu unrhyw aelod arall o'r cyhoedd) sy'n dymuno "
                          + "mynychu gwrandawiad o bell ddarparu cyfeiriad e-bost a rhif cyswllt i gael dolen briodol "
                          + "ar gyfer cael mynediad.");
    ...
    @Test
    void testKingsBenchMastersDailyCauseListTableContents() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/kingsBenchMastersDailyCauseList.json")) {
    ...
    @Test
    void testKingsBenchMastersDailyCauseListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchMastersDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, KINGS_BENCH_MASTERS_DAILY_CAUSE_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, KINGS_BENCH_MASTERS_DAILY_CAUSE_LIST, metadata,
                                                     excelFile);
    ...
    @Test
    void testKingsBenchMastersDailyCauseListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchMastersDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, WELSH,
