...
class KingsBenchDivisionDailyCauseListFileConverterTest {
    ...
    void setup() throws IOException {
        try (InputStream inputStream = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchDivisionDailyCauseList.json")) {
    ...
    @Test
    void testKingsBenchDivisionDailyCauseListFileConversionInEnglish() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/kingsBenchDivisionDailyCauseList.json")) {
        ...
        Map<String, String> metadata = Map.of(CONTENT_DATE_METADATA, CONTENT_DATE,
                                              ...
                                              LIST_TYPE_METADATA, KINGS_BENCH_DIVISION_DAILY_CAUSE_LIST.name(),
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("King’s Bench Division Daily Cause List");
    ...
    @Test
    void testKingsBenchDivisionDailyCauseListFileConversionInWelsh() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/cy/non-strategic/kingsBenchDivisionDailyCauseList.json")) {
            languageResource = new ObjectMapper().readValue(
                Objects.requireNonNull(languageFile).readAllBytes(), new TypeReference<>() {
                });
        }
        ...
        softly.assertThat(document.title())
            .as(TITLE_MESSAGE)
            .isEqualTo("Rhestr Achosion Dyddiol Adran Mainc y Brenin");
        ...
        softly.assertThat(document.getElementsByClass(SUMMARY_TEXT_CLASS).get(0).text())
            .as(IMPORTANT_INFORMATION_MESSAGE)
            .contains("Bydd y gwrandawiad ar gael i gynrychiolwyr y cyfryngau ar gais. Bydd yn cael ei drefnu a'i "
                          + "gynnal gan ddefnyddio MS Teams (oni nodir yn wahanol).");
    ...
    @Test
    void testKingsBenchDivisionDailyCauseListTableContents() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/kingsBenchDivisionDailyCauseList.json")) {
    ...
    @Test
    void testKingsBenchDivisionDailyCauseListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchDivisionDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, KINGS_BENCH_DIVISION_DAILY_CAUSE_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, KINGS_BENCH_DIVISION_DAILY_CAUSE_LIST, metadata,
                                                     excelFile);
    ...
    @Test
    void testKingsBenchDivisionDailyCauseListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/kingsBenchDivisionDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, WELSH,
