...
class MayorAndCityCivilDailyCauseListFileConverterTest {
    ...
    @Test
    void testMayorAndCityCivilDailyCauseListTableContents() throws IOException {
        Map<String, Object> languageResource;
        try (InputStream languageFile = Thread.currentThread()
            .getContextClassLoader()
            .getResourceAsStream("templates/languages/en/non-strategic/mayorAndCityCivilDailyCauseList.json")) {
    ...
    @Test
    void testMayorAndCityCivilDailyCauseListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/mayorAndCityCivilDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, MAYOR_AND_CITY_CIVIL_DAILY_CAUSE_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, MAYOR_AND_CITY_CIVIL_DAILY_CAUSE_LIST, metadata,
                                                     excelFile);
    ...
    @Test
    void testMayorAndCityCivilDailyCauseListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/mayorAndCityCivilDailyCauseList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, WELSH,
