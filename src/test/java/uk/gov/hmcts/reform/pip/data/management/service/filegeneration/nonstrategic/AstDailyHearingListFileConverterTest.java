...
class AstDailyHearingListFileConverterTest {
    ...
    @Test
    void testAstDailyHearingListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass().getResourceAsStream("/mocks/non-strategic/astDailyHearingList.xlsx");
             Workbook workbook = new XSSFWorkbook(new ByteArrayInputStream(
                 converter.convertToExcel(null, AST_DAILY_HEARING_LIST, Map.of(
                     LANGUAGE_METADATA, ENGLISH,
                     LIST_TYPE_METADATA, AST_DAILY_HEARING_LIST.name()
                 ), excelFile)
             ))) {
            ...
            SoftAssertions softly = new SoftAssertions();
            softly.assertThat(headingRow.getCell(0).getStringCellValue()).isEqualTo("Appellant");
            softly.assertThat(headingRow.getCell(1).getStringCellValue()).isEqualTo("Appeal reference number");
    ...
    @Test
    void testAstDailyHearingListExcelConversionInWelsh() throws IOException {
        try (InputStream excelFile = getClass().getResourceAsStream("/mocks/non-strategic/astDailyHearingList.xlsx");
             Workbook workbook = new XSSFWorkbook(new ByteArrayInputStream(
                 converter.convertToExcel(null, AST_DAILY_HEARING_LIST, Map.of(
                     LANGUAGE_METADATA, WELSH,
            ...
            Row headingRow = workbook.getSheetAt(0).getRow(0);
            SoftAssertions softly = new SoftAssertions();
            softly.assertThat(headingRow.getCell(0).getStringCellValue()).isEqualTo("Apelydd");
            softly.assertThat(headingRow.getCell(1).getStringCellValue()).isEqualTo("Cyfeirnod apêl");
            softly.assertThat(headingRow.getCell(2).getStringCellValue()).isEqualTo("Math o achos");
            softly.assertThat(headingRow.getCell(3).getStringCellValue()).isEqualTo("Math o wrandawiad");
            softly.assertThat(headingRow.getCell(4).getStringCellValue()).isEqualTo("Amser y gwrandawiad");
