...
class UtAdministrativeAppealsChamberDailyHearingListFileConverterTest {
    ...
    @Test
    void testAdministrativeAppealsChamberDailyHearingListExcelConversionInEnglish() throws IOException {
        try (InputStream excelFile = getClass()
            .getResourceAsStream("/mocks/non-strategic/utAdministrativeAppealsChamberDailyHearingList.xlsx")) {
            Map<String, String> metadata = Map.of(
                LANGUAGE_METADATA, ENGLISH,
                LIST_TYPE_METADATA, UT_AAC_DAILY_HEARING_LIST.name()
            );

            byte[] result = converter.convertToExcel(null, UT_AAC_DAILY_HEARING_LIST, metadata,
                                                     excelFile);
