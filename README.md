...
| MAX_PAYLOAD_SIZE               | The maximum size of input payload before we stop generating the PDF, excel and email summary for the publication. Default to 256Kb.                                                                                                                                      | No        |
| OLD_MAX_PAYLOAD_SIZE           | The maximum size of input that we should consider for deletion of generated publications. This value should be equal to MAX_PAYLOAD_SIZE, except where lowering the max payload size requires an interim period where publications may still exist within the old limit. | No        |
